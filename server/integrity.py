...
def scan(cur) -> dict:
    ...
    cur.execute(
        "delete from suspicious_activity where type in ('fast_response','pattern_match')"
    )

    fast = _flag_fast_responses(cur)
    patterns = _flag_duplicate_patterns(cur)
    return {"fast_response": fast, "pattern_match": patterns}


def _flag_fast_responses(cur) -> int:
    ...
    cur.execute(
        ...
        from quick
        """,
        (FAST_MS["hard"], FAST_MS["medium"], FAST_MS["easy"], MIN_FAST_ANSWERS),
    )
    return cur.rowcount


def _flag_duplicate_patterns(cur) -> int:
